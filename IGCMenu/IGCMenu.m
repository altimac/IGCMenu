...
{
    NSMutableArray *_menuButtonArray;        //array of menu buttons
    NSMutableArray *_menuNameLabelArray;     //array of menu name label
    UIView *_pMenuButtonSuperView;
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _menuNameLabelArray = [[NSMutableArray alloc] init];
        //Default values
...
{
    [_menuButtonArray removeAllObjects];
    [_menuNameLabelArray removeAllObjects];
    for (int i = 0; i < self.numberOfMenuItem; i++) {
        ...
        [_pMenuButtonSuperView insertSubview:menuButton belowSubview:self.menuButton];
        [_menuButtonArray addObject:menuButton];
        ...
        if (self.menuItemsNameArray.count > i) {
            ...
            newFrame.size = CGSizeMake(menuButton.frame.size.width*2, 20);
            menuNameLabel.preferredMaxLayoutWidth = menuButton.frame.size.width*2.;
            ...
            [_pMenuButtonSuperView insertSubview:menuNameLabel belowSubview:self.menuButton];
            [_menuNameLabelArray addObject:menuNameLabel];
        }
...
{
    if (_pMenuButtonSuperView == nil) {
        _pMenuButtonSuperView = [[UIView alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
        _pMenuButtonSuperView.tag = MENU_BACKGROUND_VIEW_TAG;
    }
    if (!self.menuSuperView) {
        self.menuSuperView = [self.menuButton superview];
    }
    [self.menuSuperView bringSubviewToFront:self.menuButton];
    [self.menuSuperView insertSubview:_pMenuButtonSuperView belowSubview:self.menuButton];
    
    if (self.disableBackground){
        _pMenuButtonSuperView.userInteractionEnabled = YES;
    }
    else{
        _pMenuButtonSuperView.userInteractionEnabled = NO;
    }
...
-(void)setBackgroundEffect
{
    ...
    switch (self.backgroundType) {
        case Dark:
            _pMenuButtonSuperView.layer.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.8].CGColor;
            break;
        case BlurEffectDark:
        ...
        case BlurEffectExtraLight:
            ...
            break;
        case None:
            _pMenuButtonSuperView.layer.backgroundColor = [UIColor clearColor].CGColor;
            break;
            default:
            _pMenuButtonSuperView.layer.backgroundColor = [UIColor clearColor].CGColor;
            break;
...
-(void)setBlurredView:(UIBlurEffectStyle) blurEffectStyle
{
    if (!UIAccessibilityIsReduceTransparencyEnabled()) {
        ...
        blurEffectView.frame = _pMenuButtonSuperView.bounds;
        blurEffectView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        [_pMenuButtonSuperView addSubview:blurEffectView];
    }
    else {
        _pMenuButtonSuperView.backgroundColor = [UIColor clearColor];
    }
}
...
{
    UIButton * menuButton = (UIButton *)[_menuButtonArray objectAtIndex:i/2];
    menuButton.layer.opacity = 1.0;
    CGFloat angle = self.positionStyle == Top ? M_PI / (_menuButtonArray.count * 2) : -M_PI / (_menuButtonArray.count * 2);
    ...
    if (_menuNameLabelArray.count > (i/2)) {
        UILabel *menuNameLabel = (UILabel *)[_menuNameLabelArray objectAtIndex:i/2];
...
- (void)hideCircularMenu
{
    [UIView animateWithDuration:ANIMATION_DURATION delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        for (int i = 0; i < _menuButtonArray.count; i++) {
            UIButton *menuButton = (UIButton *)[_menuButtonArray objectAtIndex:i];
            menuButton.layer.opacity = 0.0;
            menuButton.center = self.menuButton.center;
            if (_menuNameLabelArray.count > i) {
                UILabel *menuNameLabel = (UILabel *)[_menuNameLabelArray objectAtIndex:i];
                menuNameLabel.layer.opacity = 0.0;
                menuNameLabel.center = self.menuButton.center;
                _pMenuButtonSuperView.layer.opacity = 0.0;
            }
        }
    } completion:^(BOOL finished) {
        [_pMenuButtonSuperView removeFromSuperview];
        _pMenuButtonSuperView = nil;
        for (int i = 0; i < _menuButtonArray.count; i++) {
            UIButton *menuButton = (UIButton *)[_menuButtonArray objectAtIndex:i];
...
- (void)menuButtonClicked:(UIButton *)sender
{
    if ([self.delegate respondsToSelector:@selector(igcMenuSelected:atIndex:)]) {
        ...
        for (index = 0; index < _menuButtonArray.count; index++) {
            UIButton *menuButton = (UIButton *)[_menuButtonArray objectAtIndex:index];
...
- (void)orientationChanged:(NSNotification *)note
{
    ...
    switch(device.orientation) {
        ...
        case UIDeviceOrientationLandscapeRight:
            [_pMenuButtonSuperView setFrame: [UIScreen mainScreen].bounds];
